  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 bg-gradient-to-br from-blue-50 to-cyan-100">
      <div className="card max-w-sm w-full text-center">
        ...
        <p className="text-gray-500 mb-6">Tilni tanlang / �������� ����</p>
...
function RegisterPage({ lang, onDone, tgUser }: { lang: 'UZ' | 'RU'; onDone: (u: any) => void; tgUser?: any }) {
  ...
  const submit = async () => {
    if (phone.length !== 13) return toast.error(lang === 'UZ' ? "To'g'ri telefon raqam kiriting" : '������� ���������� �����');
    setLoading(true);
    try {
      const payload: any = { phone, firstName, lastName, language: lang };
      if (tgUser?.id) payload.telegramId = String(tgUser.id);
      const { data } = await registerUser(payload);
      toast.success(lang === 'UZ' ? 'Muvaffaqiyatli!' : '�������!');
      onDone(data);
    } catch {
      toast.error(lang === 'UZ' ? 'Xatolik yuz berdi' : '��������� ������');
    }
    setLoading(false);
  };
...
function BookingFlow({ user, lang, onBack }: { user: any; lang: 'UZ' | 'RU'; onBack: () => void }) {
  ...
  const confirmBooking = async () => {
    if (!selectedSlot) return toast.error(lang === 'UZ' ? 'Vaqtni tanlang' : '�������� �����');
    setLoading(true);
    try {
      ...
      toast.success(lang === 'UZ' ? 'Navbat olindi!' : '������ �������!');
      const tg = getTg();
      if (tg) { tg.showAlert(lang === 'UZ' ? 'Navbat muvaffaqiyatli olindi!' : '������ ������� �������!', () => tg.close()); }
      else onBack();
    } catch { toast.error(lang === 'UZ' ? 'Xatolik' : '������'); }
    setLoading(false);
  };
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {!isTg() && (<button onClick={goBack} className="flex items-center gap-2 text-blue-600 hover:text-blue-800 mb-4"><ArrowLeft className="w-4 h-4" /> {lang === 'UZ' ? 'Orqaga' : '�����'}</button>)}

      {step === 'region' && (
        <div>
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><MapPin className="w-6 h-6 text-blue-600" /> {lang === 'UZ' ? 'Hududni tanlang' : '�������� ������'}</h2>
      ...
      {step === 'clinic' && (
        <div>
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><Building2 className="w-6 h-6 text-green-600" /> {lang === 'UZ' ? 'Klinikani tanlang' : '�������� �������'}</h2>
          <div className="space-y-2">
            ...
            {clinics.length === 0 && <p className="text-gray-400 text-center py-8">{lang === 'UZ' ? 'Klinikalar topilmadi' : '������� �� �������'}</p>}
          </div>
        </div>
      )}
      ...
      {step === 'specialty' && (
        <div>
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><Award className="w-6 h-6 text-purple-600" /> {lang === 'UZ' ? 'Mutaxassislikni tanlang' : '�������� �������������'}</h2>
          <div className="space-y-2">
            ...
            {specialties.length === 0 && <p className="text-gray-400 text-center py-8">{lang === 'UZ' ? 'Mutaxassisliklar topilmadi' : '������������� �� �������'}</p>}
          </div>
        </div>
      )}
      ...
      {step === 'doctor' && (
        <div>
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><Stethoscope className="w-6 h-6 text-teal-600" /> {lang === 'UZ' ? 'Shifokorni tanlang' : '�������� �����'}</h2>
          <div className="space-y-3">
            {doctors.map((d: any) => (
              <button key={d.id} onClick={() => pickDoctor(d)} className="w-full text-left p-4 bg-white rounded-lg border hover:border-teal-400 hover:shadow-sm transition">
                ...
                <div className="text-sm text-gray-500 mt-1 flex flex-wrap gap-3">
                  ...
                  <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {d.experienceYears} {lang === 'UZ' ? 'yil' : '���'}</span>
                  <span className="flex items-center gap-1"><Banknote className="w-3 h-3" /> {d.price?.toLocaleString()} {lang === 'UZ' ? "so'm" : '���'}</span>
                </div>
              </button>
            ))}
            {doctors.length === 0 && <p className="text-gray-400 text-center py-8">{lang === 'UZ' ? 'Shifokorlar topilmadi' : '����� �� �������'}</p>}
          </div>
        </div>
      )}
      ...
      {step === 'date' && (
        <div>
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><Calendar className="w-6 h-6 text-indigo-600" /> {lang === 'UZ' ? 'Sana va vaqt' : '���� � �����'}</h2>
          <div className="mb-4">
            <label className="text-sm text-gray-600 mb-1 block">{lang === 'UZ' ? 'Sanani tanlang' : '�������� ����'}</label>
            <input type="date" min={today} value={selectedDate} onChange={(e) => loadSlots(e.target.value)} className="input" />
          </div>
          {selectedDate && (
            <div>
              <p className="text-sm text-gray-600 mb-2">{lang === 'UZ' ? "Bo'sh vaqtlar" : '��������� �����'}:</p>
              {slots.length > 0 ? (
                ...
                <div className="bg-red-50 border border-red-200 rounded-lg p-3 text-center"><p className="text-sm text-red-600 font-medium">{lang === 'UZ' ? 'Dam olish kuni' : '��������'}</p></div>
              )}
            </div>
          )}
        </div>
      ...
      {step === 'confirm' && selectedDoctor && (
        <div>
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><CheckCircle className="w-6 h-6 text-green-600" /> {lang === 'UZ' ? 'Tasdiqlash' : '�������������'}</h2>
          <div className="bg-white rounded-lg border p-5 space-y-3 mb-4">
            <div className="flex justify-between"><span className="text-gray-500">{lang === 'UZ' ? 'Shifokor' : '����'}</span><span className="font-medium">{selectedDoctor.firstName} {selectedDoctor.lastName}</span></div>
            <div className="flex justify-between"><span className="text-gray-500">{lang === 'UZ' ? 'Mutaxassislik' : '�������������'}</span><span className="font-medium">{sp(selectedDoctor)}</span></div>
            <div className="flex justify-between"><span className="text-gray-500">{lang === 'UZ' ? 'Klinika' : '�������'}</span><span className="font-medium">{nm(selectedClinic)}</span></div>
            <div className="flex justify-between"><span className="text-gray-500">{lang === 'UZ' ? 'Sana' : '����'}</span><span className="font-medium">{selectedDate}</span></div>
            <div className="flex justify-between"><span className="text-gray-500">{lang === 'UZ' ? 'Vaqt' : '�����'}</span><span className="font-medium">{selectedSlot}</span></div>
            <div className="flex justify-between"><span className="text-gray-500">{lang === 'UZ' ? 'Narx' : '���������'}</span><span className="font-bold text-green-600">{selectedDoctor.price?.toLocaleString()} {lang === 'UZ' ? "so'm" : '���'}</span></div>
          </div>
          <button onClick={confirmBooking} disabled={loading} className="btn-primary w-full">{loading ? '...' : lang === 'UZ' ? 'Tasdiqlash' : '�����������'}</button>
        </div>
      )}
    </div>
  );
...
function AppointmentsList({ user, lang }: { user: any; lang: 'UZ' | 'RU' }) {
  ...
  const cancel = async (appId: string) => {
    if (!confirm(lang === 'UZ' ? 'Bekor qilinsinmi?' : '��������?')) return;
    try { await cancelUserAppointment(user.id, appId); toast.success(lang === 'UZ' ? 'Bekor qilindi' : '��������'); load(); } catch { toast.error('Xatolik'); }
  };

  const badge = (s: string) => {
    const m: Record<string, { c: string; l: string }> = {
      PENDING: { c: 'bg-yellow-100 text-yellow-700', l: lang === 'UZ' ? 'Kutilmoqda' : '��������' },
      ACCEPTED: { c: 'bg-blue-100 text-blue-700', l: lang === 'UZ' ? 'Qabul qilindi' : '������' },
      COMPLETED: { c: 'bg-green-100 text-green-700', l: lang === 'UZ' ? 'Tugallandi' : '��������' },
      CANCELLED: { c: 'bg-red-100 text-red-700', l: lang === 'UZ' ? 'Bekor qilingan' : '������' },
    };
  ...
  return (
    <div>
      <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><CalendarDays className="w-6 h-6 text-blue-600" /> {lang === 'UZ' ? 'Navbatlarim' : '��� ������'}</h2>
      <div className="space-y-3">
        {list.map((a: any) => (
          <div key={a.id} className="bg-white rounded-lg border p-4">
            <div className="flex items-start justify-between">
              <div>
                ...
                <div className="text-sm text-green-600 font-medium mt-1">{(a.finalPrice ?? a.doctor?.price ?? 0).toLocaleString()} {lang === 'UZ' ? "so'm" : '���'}</div>
              </div>
              {badge(a.status)}
            </div>
            {a.status === 'COMPLETED' && a.diagnosis && (
              <div className="mt-3 bg-green-50 rounded-lg p-3 border border-green-200 text-sm space-y-1">
                <p className="text-green-800"><strong>{lang === 'UZ' ? 'Tashxis:' : '�������:'}</strong> {a.diagnosis.description}</p>
                {a.diagnosis.prescription && <p className="text-green-700"><strong>{lang === 'UZ' ? 'Dorilar:' : '������:'}</strong> {a.diagnosis.prescription}</p>}
              </div>
            )}
            {a.status === 'PENDING' && (
              <button onClick={() => cancel(a.id)} className="mt-3 text-sm text-red-500 hover:text-red-700 flex items-center gap-1"><XCircle className="w-4 h-4" /> {lang === 'UZ' ? 'Bekor qilish' : '��������'}</button>
            )}
          </div>
        ))}
        {list.length === 0 && <p className="text-gray-400 text-center py-8">{lang === 'UZ' ? 'Navbatlar topilmadi' : '������� ���'}</p>}
      </div>
    </div>
  );
}
...
function DiagnosesList({ user, lang }: { user: any; lang: 'UZ' | 'RU' }) {
  ...
  return (
    <div>
      <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><FileText className="w-6 h-6 text-green-600" /> {lang === 'UZ' ? 'Tashxislarim' : '��� ��������'}</h2>
      <div className="space-y-3">
        {list.map((d: any) => (
          <div key={d.id} className="bg-white rounded-lg border p-4">
            ...
            <p className="mt-2 text-gray-700"><strong>{lang === 'UZ' ? 'Tashxis:' : '�������:'}</strong> {d.description}</p>
            {d.prescription && <p className="mt-1 text-gray-600 text-sm"><strong>{lang === 'UZ' ? 'Dorilar:' : '������:'}</strong> {d.prescription}</p>}
          </div>
        ))}
        {list.length === 0 && <p className="text-gray-400 text-center py-8">{lang === 'UZ' ? 'Tashxislar topilmadi' : '��������� ���'}</p>}
      </div>
    </div>
  );
}
...
function UserMenu({ user, lang, onLogout }: { user: any; lang: 'UZ' | 'RU'; onLogout: () => void }) {
  ...
  if (view === 'appointments') return (
    <div className="min-h-screen bg-gray-50 p-4"><div className="max-w-2xl mx-auto">
      {!isTg() && <button onClick={() => setView('menu')} className="flex items-center gap-2 text-blue-600 mb-4"><ArrowLeft className="w-4 h-4" /> {lang === 'UZ' ? 'Orqaga' : '�����'}</button>}
  ...
  const items = [
    { key: 'booking', icon: <CalendarDays className="w-8 h-8 text-blue-600" />, title: lang === 'UZ' ? 'Navbat olish' : '����������', desc: lang === 'UZ' ? 'Shifokorga yozilish' : '������ � �����', color: 'hover:border-blue-400' },
    { key: 'appointments', icon: <ClipboardList className="w-8 h-8 text-purple-600" />, title: lang === 'UZ' ? 'Navbatlarim' : '��� ������', desc: lang === 'UZ' ? 'Barcha navbatlar' : '��� ������', color: 'hover:border-purple-400' },
    { key: 'diagnoses', icon: <FileText className="w-8 h-8 text-green-600" />, title: lang === 'UZ' ? 'Tashxislarim' : '��� ��������', desc: lang === 'UZ' ? 'Shifokor xulosalari' : '���������� ������', color: 'hover:border-green-400' },
  ];
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white shadow-sm border-b">
        <div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          {!isTg() && <button onClick={onLogout} className="flex items-center gap-2 text-red-500 hover:text-red-700 text-sm"><LogOut className="w-4 h-4" /> {lang === 'UZ' ? 'Chiqish' : '�����'}</button>}
        </div>
      </header>
