...
import {
  registerUser, getUserByPhone, getUserAppointments, getUserDiagnoses,
  cancelUserAppointment, getRegions, getClinicsByRegion, getClinicSpecialties,
  getDoctorsByClinicAndSpecialty, getDoctorDetail, getDoctorSlots, createAppointment,
} from '../../lib/api';
import {
  User, Phone, Globe, ArrowLeft, Calendar, Building2, Stethoscope,
  Clock, Star, Award, Banknote, MapPin, LogOut, CalendarDays,
  ClipboardList, FileText, CheckCircle, XCircle, AlertCircle,
} from 'lucide-react';
...
/* ════════════════════════ REGISTER ════════════════════════ */
function RegisterPage({ lang, onDone }: { lang: 'UZ' | 'RU'; onDone: (u: any) => void }) {
  ...
  const submit = async () => {
    if (phone.length !== 13) return toast.error(lang === 'UZ' ? "To'g'ri telefon raqam kiriting: +998XXXXXXXXX" : 'Введите корректный номер: +998XXXXXXXXX');
    setLoading(true);
    try {
      const { data } = await registerUser({ phone, firstName, lastName, language: lang });
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      {step === 'region' && (
        <div>
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
            <MapPin className="w-6 h-6 text-blue-600" /> {lang === 'UZ' ? 'Hududni tanlang' : 'Выберите регион'}
          </h2>
          <div className="space-y-2">
            {regions.map((r: any) => (
              <button key={r.id} onClick={() => pickRegion(r)} className="w-full text-left p-4 bg-white rounded-lg border hover:border-blue-400 hover:shadow-sm transition">
                <span className="font-medium">{nm(r)}</span>
              </button>
            ))}
          </div>
        </div>
      ...
      {step === 'clinic' && (
        <div>
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
            <Building2 className="w-6 h-6 text-green-600" /> {lang === 'UZ' ? 'Klinikani tanlang' : 'Выберите клинику'}
          </h2>
          <div className="space-y-2">
            {clinics.map((c: any) => (
              <button key={c.id} onClick={() => pickClinic(c)} className="w-full text-left p-4 bg-white rounded-lg border hover:border-green-400 hover:shadow-sm transition">
      ...
      {step === 'specialty' && (
        <div>
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
            <Award className="w-6 h-6 text-purple-600" /> {lang === 'UZ' ? 'Mutaxassislikni tanlang' : 'Выберите специальность'}
          </h2>
          <div className="space-y-2">
            {specialties.map((s: any) => (
              <button key={s.id} onClick={() => pickSpecialty(s)} className="w-full text-left p-4 bg-white rounded-lg border hover:border-purple-400 hover:shadow-sm transition font-medium">
                {nm(s)}
              </button>
            ))}
      ...
      {step === 'date' && (
        <div>
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
            <Calendar className="w-6 h-6 text-indigo-600" /> {lang === 'UZ' ? 'Sana va vaqt' : 'Дата и время'}
          </h2>
          <div className="mb-4">
              ...
              {slots.length > 0 ? (
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
                  {slots.map((s: any) => {
                    ...
                    return (
                      <button
                        key={label}
                        onClick={() => { setSelectedSlot(label); setStep('confirm'); }}
                ...
                <div className="bg-red-50 border border-red-200 rounded-lg p-3 text-center">
                  <p className="text-sm text-red-600 font-medium">
                    {lang === 'UZ' ? "Bu kuni shifokor qabul qilmaydi — dam olish kuni" : "Врач не принимает в этот день — выходной"}
                  </p>
                </div>
              )}
            </div>
      ...
      {step === 'confirm' && selectedDoctor && (
        <div>
          <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
            <CheckCircle className="w-6 h-6 text-green-600" /> {lang === 'UZ' ? 'Tasdiqlash' : 'Подтверждение'}
          </h2>
          <div className="bg-white rounded-lg border p-5 space-y-3 mb-4">
            <div className="flex justify-between">
              <span className="text-gray-500">{lang === 'UZ' ? 'Shifokor' : 'Врач'}</span>
              <span className="font-medium">{selectedDoctor.firstName} {selectedDoctor.lastName}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">{lang === 'UZ' ? 'Mutaxassislik' : 'Специальность'}</span>
              <span className="font-medium">{sp(selectedDoctor)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">{lang === 'UZ' ? 'Klinika' : 'Клиника'}</span>
              <span className="font-medium">{nm(selectedClinic)}</span>
            </div>
            <div className="flex justify-between">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-bold text-green-600">{selectedDoctor.price?.toLocaleString()} {lang === 'UZ' ? "so'm" : 'сум'}</span>
            </div>
          </div>
          <button onClick={confirmBooking} disabled={loading} className="btn-primary w-full">
            {loading ? '...' : lang === 'UZ' ? 'Tasdiqlash' : 'Подтвердить'}
          </button>
        </div>
      )}
    </div>
...
function AppointmentsList({ user, lang }: { user: any; lang: 'UZ' | 'RU' }) {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try { const { data } = await getUserAppointments(user.id); setList(data); }
  ...
  const cancel = async (appId: string) => {
    ...
    try { await cancelUserAppointment(user.id, appId); toast.success(lang === 'UZ' ? 'Bekor qilindi' : 'Отменено'); load(); }
    catch { toast.error('Xatolik'); }
  };

  const statusBadge = (s: string) => {
    const m: Record<string, { color: string; icon: React.ReactNode; label: string }> = {
      PENDING: { color: 'bg-yellow-100 text-yellow-700', icon: <AlertCircle className="w-3 h-3" />, label: lang === 'UZ' ? 'Kutilmoqda' : 'Ожидание' },
      ACCEPTED: { color: 'bg-blue-100 text-blue-700', icon: <CheckCircle className="w-3 h-3" />, label: lang === 'UZ' ? 'Qabul qilindi' : 'Принят' },
  ...
  return (
    <div>
      <h2 className="text-xl font-bold mb-4 flex items-center gap-2">
        <CalendarDays className="w-6 h-6 text-blue-600" /> {lang === 'UZ' ? 'Navbatlarim' : 'Мои записи'}
      </h2>
      <div className="space-y-3">
        {list.map((a: any) => (
          <div key={a.id} className="bg-white rounded-lg border p-4">
            <div className="flex items-start justify-between">
              <div>
                ...
                {(a.finalPrice != null || a.doctor?.price != null) && (
                  <div className="text-sm text-green-600 font-medium mt-1">
                    {(a.finalPrice ?? a.doctor?.price ?? 0).toLocaleString()} {lang === 'UZ' ? "so'm" : 'сум'}
                  </div>
                )}
              </div>
            ...
            {a.status === 'COMPLETED' && a.diagnosis && (
              <div className="mt-3 bg-green-50 rounded-lg p-3 border border-green-200 space-y-1 text-sm">
                <p className="text-green-800"><strong>{lang === 'UZ' ? 'Tashxis:' : 'Диагноз:'}</strong> {a.diagnosis.description}</p>
                {a.diagnosis.prescription && (
                  <p className="text-green-700"><strong>{lang === 'UZ' ? 'Dorilar:' : 'Рецепт:'}</strong> {a.diagnosis.prescription}</p>
                )}
              </div>
            )}
            {a.status === 'PENDING' && (
              <button onClick={() => cancel(a.id)} className="mt-3 text-sm text-red-500 hover:text-red-700 flex items-center gap-1">
                <XCircle className="w-4 h-4" /> {lang === 'UZ' ? 'Bekor qilish' : 'Отменить'}
              </button>
            )}
          </div>
        ))}
